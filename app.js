...
document.getElementById('fileInput').addEventListener('change', async (e) => {
  ...
  for (const file of e.target.files) {
    ...
    try {
      ...
      await supabase.from('feed_reports').upsert([{ filename: file.name, data: json }]);
    } catch {
...
function updateFileList() {
  ...
  list.innerHTML = names.length ? names.map(n => `<div>${n}</div>`).join('') : '<p>No files loaded.</p>';
}
...
function renderReport(data) {
  ...
  if (!data.length) return (reportTable.style.display = 'none');
    ...
    groups[date].forEach(entry => {
      ...
      tr.innerHTML = `
        ...
        <td class="action-column">
          <button class="edit-btn" onclick="editEntry(${entry._index})">Edit</button>
          <button class="remove-btn" onclick="removeEntry(${entry._index})">Remove</button>
        </td>`;
...
async function removeEntry(index) {
  ...
  currentFiles[currentFileName].splice(index, 1);
  renderReport(currentFiles[currentFileName]);
